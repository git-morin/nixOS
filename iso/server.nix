...
let
  proxmoxConfig = import ../hosts/server/proxmox.nix;
...
#  services.proxmox-ve.enable = lib.mkForce false;
  ...
  networking = {
    hostName = "nixos-server-installer";
  ...
  environment.etc."proxmox-install-script" = {
    text = ''
      ...
      echo "Server NixOS Installation Guide"
      echo "================================"
      echo ""
      echo "Steps:"
  ...
  # ISO image settings
  isoImage = {
    isoName = "nixos-server-installer.iso";
    volumeID = "NIXOS_SERVER";
    isoBaseName = lib.mkForce "nixos-server";
